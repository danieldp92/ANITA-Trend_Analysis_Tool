...
def p_product_name(soup):
    ...
    return soup.find('h1', {'class' : 'product-title product_title entry-title'}).text
...
def p_price(soup):
    ...
    return soup.find('div', {'class' : 'price-wrapper'}).text


def p_info(soup):
    """Return the info as string"""
    # example: return soup.find('div', {'prod_info'}).text
    return soup.find('div', {'class' : 'product-short-description'}).text

def p_macro_category(soup):
    """Return a macro category, for example: drugs, weapons, other"""
    return 'drugs'  # Only drugs sold on this website

def p_micro_category(soup):
    """Return a micro category, for example: cannabis, cocaine"""
    return soup.find('nav', {'class' : 'woocommerce-breadcrumb breadcrumbs uppercase'}).find_all('a')[-1].text
...
def p_feedback(soup):
    ...
    #loop to walk through the feedback
    reviews = soup.find('div', {'id': 'comments'}).find_all('li')
    for review in reviews:
        ...
        # The message of the feedback in type str
        message =  soup.find('div', {'class' : 'description'}).text

        # The time in datetime object or time ago in type str
        date = parse(soup.find('time', {'class' : 'woocommerce-review__published-date'}).text)

        # User, name of the user or encrypted user name (if any) in type str
        user = soup.find('strong', {'class' : 'woocommerce-review__author'}).text
